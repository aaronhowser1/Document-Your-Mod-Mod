...
public final class StainedPlanksEnabledConditionFactory implements ConditionFactory {
    @Nonnull
    @Override
    ...
    private boolean reflectInto() {
        try {
            final Class<?> target = Class.forName("vazkii.quark.building.feature.VerticalWoodPlanks");
            final Field targetField = target.getDeclaredField("enableVerticalStained");
            ...
            Objects.requireNonNull(ApiBindings.getMainApi().getCurrentLoadingState())
                    .getReporter().interrupt("Unable to determine stained planks situation! Returning false to be safe!");
            return false;
        }
    }
}
