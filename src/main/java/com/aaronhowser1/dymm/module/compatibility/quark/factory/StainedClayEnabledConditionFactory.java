...
public final class StainedClayEnabledConditionFactory implements ConditionFactory {
    ...
    private boolean reflectInto() {
        try {
            final Class<?> target = Class.forName("vazkii.quark.building.feature.HardenedClayTiles");
            final Field targetField = target.getDeclaredField("enableStainedClay");
            ...
            Objects.requireNonNull(ApiBindings.getMainApi().getCurrentLoadingState())
                    .getReporter().interrupt("Unable to determine stained clay situation! Returning false to be safe!");
            return false;
        }
    }
}
