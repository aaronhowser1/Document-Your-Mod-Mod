...
public final class WraithCurseStatusConditionFactory implements ConditionFactory {
    @Nonnull
    @Override
    ...
    private boolean reflectInto() {
        try {
            final Class<?> target = Class.forName("vazkii.quark.world.feature.Wraiths");
            final Field targetField = target.getDeclaredField("enableCurse");
            ...
            Objects.requireNonNull(ApiBindings.getMainApi().getCurrentLoadingState())
                    .getReporter().interrupt("Unable to determine curse status! Returning false to be safe!");
            return false;
        }
    }
}
